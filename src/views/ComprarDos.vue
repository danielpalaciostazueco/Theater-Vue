<template>
    <div>
      <main>
        ...
        <section class="frame-function" v-if="obra && obra.Imagenes.length > 0">
        ...
        <section class="information-section">
          ...
          <ul class="horarios-list">
            <li v-for="fecha in obra?.Fechas" :key="fecha" class="horarios-item">
              {{ fecha }}
              <RouterLink :to="{ name: 'comprarEntrada', params: { obraId: obra?.ObraId, fecha: fecha }}" class="button-comprar">Comprar</RouterLink>
            </li>
          </ul>
        </section>
        <article>
          ...
          <div class="frame-reparto">
            <h2>Reparto</h2>
            <ul>
              <li v-for="actor in obra?.Actores" :key="actor">{{ actor }}</li>
            </ul>
          </div>
        </article>
      </main>
